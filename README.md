...
Then I copied the following into a file named `index.html` in the `/var/www/ferretcam/public_html` directory:
...
<button class="btnDefault" id="play">Start</>
<script>
'use strict';
...
I ran VLC as above manually once more and browsed to http://ioc-server.redirectme.net:8080/ then pressed the red `START` button to make the stream play (the reason for the button is that, unless you are privileged like YouTube or similar, mobile and sometimes desktop browsers will not play a video stream without user interaction).
